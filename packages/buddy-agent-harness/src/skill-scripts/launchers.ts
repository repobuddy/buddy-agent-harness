/**
 * Which subcommand each shipped skill's script bundle runs. Keyed by skill rather than derived from
 * it — a skill's name does not name its subcommand: `repair` runs `doctor` to find what it repairs,
 * and `init-buddy-agent-harness` needs both — `init` to write the projections, and `doctor` to list
 * the artifacts only one harness can read.
 *

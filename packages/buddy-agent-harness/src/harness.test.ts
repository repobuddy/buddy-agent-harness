...
describe('initializeHarnesses', () => {
	it('always links canonical skills for Claude Code and records the enabled harness', () => {
		...
		expect(JSON.parse(readFileSync(join(root, '.agents', 'buddy-agent-harness', 'config.json'), 'utf8'))).toEqual({
	...
	it('uses registry detection and --copy for harnesses already present in the repository', () => {
		const root = repository()
		for (const directory of ['.cursor', '.codex', '.github/skills', '.windsurf'])
	...
	it('preflights every conflict before writing and replaces them only with --force', () => {
		...
		expect(() => initializeHarnesses({ root })).toThrow(/\.claude\/skills\/review/)
		expect(existsSync(join(root, '.cursor', 'review'))).toBe(false)

		initializeHarnesses({ root, force: true })
		expect(lstatSync(join(root, '.claude', 'skills', 'review')).isSymbolicLink()).toBe(true)
		expect(lstatSync(join(root, '.cursor', 'review')).isSymbolicLink()).toBe(true)
	})

	it('preserves a canonical link and rejects a link to another skill directory', () => {
		const root = repository()
		...
		const target = join(conflictingRoot, '.claude', 'skills', 'review')
		mkdirSync(join(conflictingRoot, '.claude', 'skills'), { recursive: true })
		symlinkSync('../../not-review', target, 'junction')

		expect(() => initializeHarnesses({ root: conflictingRoot })).toThrow(/\.claude\/skills\/review/)
	})
})

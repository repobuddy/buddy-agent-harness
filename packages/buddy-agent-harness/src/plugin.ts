...
function write(value: object, format: "json" | "toon" | undefined): void {
	process.stdout.write(
		`${format === "json" ? JSON.stringify(value) : encode(value)}\n`,
	);
}
...
export const initCommand: cli.Command = command({
	name: "init",
	description:
		"Set up canonical agent skills for the harnesses already enabled in a repository.",
	options: {
		root: {
			description:
				"Repository or package directory. Defaults to the current directory.",
	...
	run(args) {
		try {
			...
			if (format !== "toon" && format !== "json")
				throw new Error("--format must be toon or json.");
			write(
				initializeHarnesses({
			...
			process.stderr.write(
				`error: ${error instanceof Error ? error.message : "Harness initialization failed."}\n`,
			);
			process.exitCode = 1;
		}
	},
...
export function activate({
	...
	addCommand(command: typeof harnessCommand): void;
}): void {

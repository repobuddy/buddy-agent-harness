import { cpSync, lstatSync, mkdirSync, readdirSync, readlinkSync, rmSync, symlinkSync, writeFileSync } from 'node:fs'
import { dirname, join, relative } from 'node:path'

export type HarnessName = 'claude-code' | 'cursor' | 'codex' | 'copilot-cli' | 'windsurf'
...
function selectedHarnesses(root: string): Harness[] {
	return harnessRegistry.filter((harness) => !harness.detect || directoryExists(join(root, harness.detect)))
}

export function initializeHarnesses({ root, copy = false, force = false }: InitializeOptions): InitializeResult {
	...
	const conflicts = harnesses.flatMap((harness) =>
		skills
			...
			.filter(({ target, source }) => pathOccupied(target) && !isCorrectLink(target, source))
			.map(({ target }) => target),
	)

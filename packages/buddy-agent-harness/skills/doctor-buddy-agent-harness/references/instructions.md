...
| --- | --- | --- |
| `no-instructions` | no AGENTS.md at the repository root, so every instruction bridge points at nothing | run `/buddy-agent-harness:init-buddy-agent-harness`, which derives AGENTS.md and the bridges to it |

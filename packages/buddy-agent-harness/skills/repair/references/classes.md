...
**Who to hand one to is read off the repair `doctor` gave it, and nowhere else.** A repair naming `/buddy-agent-harness:init` goes to `init` — it writes both kinds of bridge in the first place, and it writes a `CLAUDE.md` stub *without* asking, where every correction here needs approval. A repair naming **no skill** is work for a person: `doctor` states it in full, and passing it on is the whole of what you do with it. Every MCP finding is that second case, as is every bridge finding a rebuild would not fix.

...
export default defineConfig([
	...
	},
	{
		...
		dts: false,
		// `jsonc-parser` (reached through clibuilder) resolves to its UMD build via
		// `main`, and that build's factory calls `require("./impl/format")` and three
		// siblings — specifiers rolldown cannot analyse. So its `impl/` modules were
		// never inlined, and at runtime the require resolved against `dist/` and threw
		...
		// extensionless specifiers are fine because a bundler resolves them, which is
		// what its `module` entry exists for.

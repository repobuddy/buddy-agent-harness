...
Most repositories do not start empty. They have a `CLAUDE.md`, a `.cursor/rules/` tree, a `.claude/skills/` directory, or all three. Migration is the part of [initialization](/skills/init-buddy-agent-harness/) that needs judgment, which is why the skill handles it and the CLI does not.

...
| `managed` | the machine-wide directory: `/etc/universal-plugin/governances` on Linux, `/Library/Application Support/UniPlugin/governances` on macOS, `%ProgramData%\UniPlugin\governances` on Windows |
| `package` | governances this package ships. It ships none today. |

The `managed` layer is a machine-wide default, not enforcement — it sits after the project and user
layers, so either can override it. Its directory kept the name `universal-plugin` deliberately: the
layer moved to this package, but the location did not, so a machine already carrying one keeps being
read.
...
Restricts resolution to the `project`, `user`, and `managed` layers, and never returns a governance
this package ships. This is the guarantee behind step 2 of the lookup order a skill runs at load
...
plus the `layers` section naming every layer in lookup order. Always exits `0`.
...
layers:
  ...
  user     ~/.agents/governances
  managed  /etc/universal-plugin/governances
...
| `--overrides-only` | Resolve only the project, user, and machine-wide layers. Never returns a governance this package ships. |

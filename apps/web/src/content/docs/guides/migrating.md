...
| Subagents | `.claude/agents/` | Canonical-only — no cross-harness format exists. Reported, left in place |
...
Harness instruction files are prose, and prose merges. Their content moves into the root `AGENTS.md` with the author's wording preserved — appended, not restructured — and the harness file is replaced with a pointer only where you approve it.
...
It does not. `.mdc` and `.md` are not interchangeable, path-scoped rules have no `AGENTS.md` equivalent, and [Cursor reads `AGENTS.md` in Agent mode only](/agent-configuration/harnesses/cursor/) — Chat and Composer still read the rules files. A repository that consolidates into `AGENTS.md` and deletes its rules loses instructions in two of three Cursor surfaces.
...
Move those skills into `.agents/skills/` first. Do not clear the conflict with `--force` — that discards them.

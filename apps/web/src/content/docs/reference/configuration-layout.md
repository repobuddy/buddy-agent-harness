...
Repository instructions are the middle case. `AGENTS.md` is canonical, and one harness cannot read it without a bridge the CLI does not write, because it needs judgment about user-authored content: the [Gemini CLI](/agent-configuration/harnesses/gemini-cli/) `context.fileName` edit. The other half of the same judgment is subtractive — a [`CLAUDE.md` that suppresses the `AGENTS.md` beside it](/agent-configuration/harnesses/claude-code/#a-claudemd-beside-it-suppresses-it) holds content someone wrote, and consolidating it is not a mechanical move either. The [`init-buddy-agent-harness` skill](/skills/init-buddy-agent-harness/) handles both.

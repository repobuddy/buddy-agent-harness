...
**governance**: a version-pinned Markdown rule set that a skill loads, such as `skill-design` or `agent-tool-output`. Each skill ships its own committed copy; the project, user, and machine-wide layers under `.agents/governances/` can override it. See [`governance`](/cli/governance/).

...
The `enhance` skill proposes guidance a repository does not have, and offers a fresh copy of guidance it took from an earlier release. [`init-buddy-agent-harness`](/skills/init-buddy-agent-harness/) is the other half: it consolidates what you already wrote and bridges the harnesses that cannot read it, and it invents nothing. Initialization has to be safe to run anywhere, so it carries no opinions. An addition is opinionated by construction.
...
The target is the root `AGENTS.md`. If there is none, the skill stops and points at [`init-buddy-agent-harness`](/skills/init-buddy-agent-harness/), because it adds to an existing file and creating one is `init`'s job. A nested `AGENTS.md` is never a target, since none of the additions are scoped to a subtree.
...
- Never consolidate harness instruction files. Reading them is the coverage judgment; merging them is [`init-buddy-agent-harness`](/skills/init-buddy-agent-harness/).

...
`listMcpServers` reads further than this page's table, because an inventory has no golden set to compare against and nothing to diagnose — only `doctor`'s project-scope-only policy above is unchanged. Beyond the harnesses `harnessRegistry` documents, it also reads: VS Code's user-scope `mcp.json` (`servers`) and project-scope `.vscode/mcp.json`; Windsurf's legacy `~/.codeium/windsurf/mcp_config.json`, read under `devin-desktop` since Windsurf was rebranded to Devin Desktop (`.research/agentic-configuration-standards/evidence.md`, E-WS-02); OpenCode's `opencode.json` (`mcp`) at both scopes; Zed's `settings.json` (`context_servers`) at both scopes; Claude Code's project-local servers nested in `~/.claude.json` under `projects["<project dir>"].mcpServers` (`scope: 'local'`); and the MCP servers an installed Claude Code plugin ships, gated on whether that plugin is enabled for the project (`scope: 'plugin'`, with the plugin id on `McpServerEntry.plugin`).

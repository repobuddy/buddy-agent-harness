...
| Agent Skills lenient validation — the skill is skipped | the skill does not load at all, even by name |

A fixed marker string is the only value that is unmatchable in the first case and present in the second.

Because of that workaround, a Direct skill is not suitable as a slash command: the `/` menu would show only the marker, telling the user nothing. That is why a Direct skill typically also carries `user-invocable: false` where the harness supports it — a menu tidiness measure, not the mechanism.
...
1. Try to load a skill literally named `reviewer` — a project or package may have defined one to override the default.

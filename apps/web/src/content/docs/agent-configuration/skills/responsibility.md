...
description: The Required / Optional / Delegated axis, covering what an agent-configuration artifact must do itself, may optionally add, or hands to a named role via dependency inversion.
---
...
|---|---|---|---|
| [Gateway skill](/agent-configuration/skills/gateway-skill/) | Activation, intake against a fixed operation menu, context loading, routing | Continuing to shape the work after routing | Voice and judgment during intake and routing, via an optional by-name persona |
| [Persona](/agent-configuration/skills/persona/) | Identity layer (role, expertise, voice) and capability layer (tools, constraints) | Additional constraints beyond the defaults | None; a persona is typically the delegate, not the delegator |

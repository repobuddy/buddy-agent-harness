...
This is often described as deprecated. That is not what the vendor documentation says: Claude Code states that custom commands have been **merged** into skills, that `.claude/commands/deploy.md` and `.claude/skills/deploy/SKILL.md` both create `/deploy` and work the same way, and that existing `commands/` files keep working. Skills are the superset — they add a directory for supporting files, invocation-control frontmatter, and automatic loading — but the directory is a supported equivalent, not a scheduled removal.
...
- `.claude/commands/` is a Claude Code path. The Agent Skills specification says nothing about commands, and `.agents/commands/` is not read by anything — adopting it would be inventing a convention rather than following one.

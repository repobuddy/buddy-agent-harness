...
Antigravity reads `<workspace-root>/.agents/skills/` and needs nothing written for it. Gemini CLI reads `.gemini/skills/` at project scope and needs a projection — `.agents/skills` is a user-scope alias there only. Enabling one says nothing about the other.
...
The directory-level link deserves a note on its contract status. Claude Code documents symlinks at the *per-skill* level: a `<skill-name>` entry may point elsewhere on disk, and the skill is loaded once even when reachable from several locations. Symlinking the `.claude/skills` **directory itself** is undocumented, but verified working. It is preferred because it is live — a skill added to `.agents/skills/` later appears without re-running anything — while per-skill links remain the documented fallback. Treat it as supported in practice rather than guaranteed by contract.
